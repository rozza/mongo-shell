  ...
  executeAsync(cmd, context = {}, options = {}) {
    ...
    var output = falafel(cmd, function (node) {
      if (node.type === 'CallExpression') {
        ...
        node.update(`yield __shellWrapperMethod(${detectCallbacks}, "${node.callee.source()}", ${node.callee.source()}).apply(${node.callee.object ? node.callee.object.source() : 'this'}, [${node.arguments.map(x => x.source())}])`);
