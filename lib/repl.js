...
class REPL extends EventEmitter {
  ...
  __writer(line) {
    ...
    if(line != null && typeof line == 'object') {
      if(typeof line.render == 'function') {
        line = JSON.stringify(line.render('repl'), null, 2);
      } else {
  ...
  __eval(cmd, context, filename, callback) {
    ...
    co(function*() {
      ...
      // } else if (self.context.__result
      //   && typeof self.context.__result.render == 'function') {
      //     finalResult = self.context.__result.render('repl');
      }
